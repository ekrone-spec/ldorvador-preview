...
(function(){
  ...
  if (!f) return;
  var bedsField = f.querySelector('.gbeds');
  var bedsInputs = bedsField ? bedsField.querySelectorAll('input[name="beds"]') : null;
  function syncBeds(){
    if (!bedsField || !bedsInputs) return;
    var room = f.querySelector('input[name="room"]:checked');
    var isDouble = !!room && room.value === 'double';
    bedsField.hidden = !isDouble;
    for (var i=0;i<bedsInputs.length;i++){
      bedsInputs[i].required = isDouble;
      if (!isDouble) bedsInputs[i].checked = false;
    }
  }
  f.addEventListener('change', function(e){
    if (e.target && e.target.name === 'room') syncBeds();
  });
  syncBeds();
...
/* Group trip page: the day-by-day flow is styled as an always-open staggered
   list, not an accordion (CSS alone can't force this \u2014 a closed <details>'s
   non-summary content is skipped from the parent's own layout sizing by the
   browser even when display is overridden, which collapses the day box and
   overlaps the next day). Force every day open on load and ignore clicks on
   its heading so it can never collapse. */
document.querySelectorAll('details.group-day').forEach(function(d){
  d.open = true;
