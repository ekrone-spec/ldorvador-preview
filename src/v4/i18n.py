...
class _Walk(HTMLParser):
    ...
    def _flush(self):
        ...
        if self.collect is not None:
            if not re.fullmatch(r'[\s\W\d]+', stripped) and stripped not in self.collect:
                self.collect.append(stripped)
        self.buf.append(lead + self.map.get(stripped, stripped) + tail)
